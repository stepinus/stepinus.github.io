...
void main() {
    ...
    if (isPoint > 0.5) {
        if (isWaveSizeActive) {
            // Уменьшаем масштаб волн и добавляем смещение
            float waveTime = time * waveSpeed;
            ...
            // Добавляем пороговое значение для создания более редких волн
            float threshold = 0.005;
            float waveEffect = smoothstep(threshold, 1.0, abs(noiseValue));

            // Вычисляем размер точки на основе порогового шума
            float pointSize = baseParticleSize + waveSizeScale * waveEffect * 0.5;
            pointSize *= 1.0 + audioIntensity * 1.5;// Уменьшаем влияние аудио
            ...
            vSizeFactor = 0.0;
            gl_PointSize = size * (300.0 / -mvPosition.z) * (1.0 + audioIntensity * 2.0);// Немного уменьшаем влияние audioIntensity
            vGradientFactor = 0.0;
        }
