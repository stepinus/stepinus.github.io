...
void main() {
    ...
    // Модифицируем деформацию с учетом аудио
    if(isDeformActive) {
        float noiseValue = cnoise(vec3(position.x * frequency + time, position.y * frequency + time, position.z * frequency + time));
        vec3 deformation = vec3(noiseValue) * sin(time * 2.0 + audioBass * 10.0) * amplitude * intensity * audioIntensity;
        newPosition += deformation;
        // vDeformationFactor = (noiseValue + 1.0) * 0.5 * audioIntensity;
    } else {
    ...
    // Модифицируем размер точек с учетом аудио
    if(isPoint > 0.5) {
        if(isWaveSizeActive) {
            float waveTime = time * waveSpeed * (1.0 + audioTreble);
            ...
            float pointSize = baseParticleSize + waveSizeScale * (noiseValue * 0.5 + 0.5) * (1.0 + audioIntensity);

            gl_PointSize = pointSize * (300.0 / -mvPosition.z);

            vSizeFactor = (noiseValue * 0.5 + 0.5) * audioIntensity;
            vGradientFactor = vSizeFactor;
        } else {
            vSizeFactor = 0.0;
            gl_PointSize = size * (300.0 / -mvPosition.z) * (1.0 + audioIntensity * 0.5);
            vGradientFactor = 0.0;
        }
    } else {
        vSizeFactor = 0.0;
