...
void main() {
    ...
    if(isPoint > 0.5) {
        if(isWaveSizeActive) {
            float waveTime = time * waveSpeed;
            vec3 noisePosition = vec3(newPosition.x * waveScale + waveTime,
            newPosition.y * waveScale + waveTime,
            newPosition.z * waveScale + waveTime);
            float noiseValue = cnoise(noisePosition);

            float pointSize = baseParticleSize + waveSizeScale * (noiseValue * 0.5 + 0.5);
            pointSize *= 1.0 + audioIntensity * 3.0;
        ...
        // Для линий не устанавливаем размер точки
        vSizeFactor = 0.0;
    }
