...
void main() {
    ...
    gl_FragColor.rgb += color * 0.5 * (1.0 + audioIntensity * 0.5);

    // Добавляем мерцание на основе аудио интенсивности
    gl_FragColor.rgb *= 1.0 + audioIntensity * 0.2 * sin(gl_FragCoord.x * 0.1 + gl_FragCoord.y * 0.1);
