...
void main() {
    ...
    if(vIsPoint > 0.5) {
        ...
        vec3 color = mix(baseColor, waveColor, 1.0);
        color *= (diff + ambient) * brightness * 0.5; // Применяем освещение и яркость
        // float alpha texColor.a * 0.5;
        gl_FragColor = vec4(color, texColor.a);
        if(gl_FragColor.a < 0.3)
            discard;
    } else {
        vec3 lineColor = baseColor * (diff + ambient) * brightness * 0.2; // Применяем освещение и яркость к линиям
